...
for n4 = 1:3
    for Nmm = 0.4:0.1:1
        ...
        %         index = randi([1,size(X,1)],[1,Nm]);
                ...
                for i = 1:Nm
                    disp(i);
                    X1 = X(index(i),1:Nr)';%第1个网络在第t时刻的时间序列值
                    X2 = X(index(i),Nr+1:end)';%第2个网络在第t时刻的时间序列值
                    F1 = zeros(size(X1));
                    for j = 1:N
                        if strcmp(name3, 'L-L')
                    ...
                    XX1 = (X(index(i)+1,1:Nr)'-X(index(i)-1,1:Nr)')./(2*t0);%第1个网络在第t时刻的时间序列值对应的近似微分，使用中心差商公式
                    XX2 = (X(index(i)+1,Nr+1:end)'-X(index(i)-1,Nr+1:end)')./(2*t0);%第2个网络在第t时刻的时间序列值对应的近似微分，使用中心差商公式
%                     XX1 = F1 - kron((D1*L1+D12*eye(N)),daota)*X1 + kron(D12*eye(N),daota)*X2;%真实的XX1
%                     XX2 = F2 - kron((D2*L2+D21*eye(N)),daota)*X2 + kron(D21*eye(N),daota)*X1;%真实的XX2
